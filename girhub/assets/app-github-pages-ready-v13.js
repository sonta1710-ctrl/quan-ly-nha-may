...
window.openScanInput=function(){const val=$("scanInput")?.value||""; const mid=parseMachineId(val); if(mid) return location.href=`/machine/${encodeURIComponent(mid)}`; const did=parseDepartmentId(val); if(did) return location.href=`/department/${encodeURIComponent(did)}`; alert("Mã không hợp lệ.");}
...
function drawAdminDotsOnly(){
  ...
  dots.innerHTML=filtered.map(m=>{const cls=`dot ${m.status} ${appState.selectedMachineId===m.id?"selected":""}`; return `<a class="${cls}" href="/machine/${encodeURIComponent(m.id)}" data-id="${m.id}" style="left:${m.x}%;top:${m.y}%" title="${m.id} - ${m.name} - ${m.departmentName||m.area}"></a>`;}).join("");
